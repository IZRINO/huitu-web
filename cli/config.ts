...
import type { Params, Settings } from '../src/types.js'
import { CliError } from './types.js'
import type { Config, Profile, JobSpec } from './types.js'
...
export function object(value: unknown, name = 'input'): asserts value is Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new CliError(`${name} must be an object`)
}
export function keys(value: Record<string, unknown>, allowed: string[], name: string) {
  for (const key of Object.keys(value)) if (!allowed.includes(key)) throw new CliError(`Unknown ${name} field: ${key}`)
}
export function integer(value: unknown, min: number, max: number, name: string) {
  if (!Number.isInteger(value) || Number(value) < min || Number(value) > max) throw new CliError(`${name} must be an integer between ${min} and ${max}`)
}
export function text(value: unknown, name: string): asserts value is string {
  if (typeof value !== 'string') throw new CliError(`${name} must be a string`)
}
export function url(value: string, name: string) {
  try { const u = new URL(value); if (!['http:', 'https:'].includes(u.protocol) || u.username || u.password) throw new Error() }
  catch { throw new CliError(`${name} must be an absolute HTTP(S) URL without embedded credentials`) }
}
export function profileName(value: string) {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_.-]{0,63}$/.test(value) || ['__proto__', 'constructor', 'prototype'].includes(value)) throw new CliError('Invalid profile name')
}
...
export function validateParams(input: unknown): asserts input is Partial<Params> {
  object(input, 'params'); keys(input, Object.keys(defaultParams()), 'params')
  for (const [key, value] of Object.entries(input)) {
    if (choices[key]) { if (!choices[key].includes(value as string)) throw new CliError(`Invalid ${key}: ${value}`) }
    else if (key === 'stream') { if (typeof value !== 'boolean') throw new CliError('stream must be boolean') }
    else if (key === 'sizePreset') text(value, key)
    else if (key === 'n') integer(value, 1, 10, key)
    else if (key === 'compression') integer(value, 0, 100, key)
    else if (key === 'partialImages') integer(value, 0, 3, key)
    else integer(value, 16, 3840, key)
  }
}
export function validateSettings(input: unknown): asserts input is Partial<Settings> {
  ...
  for (const [key, value] of Object.entries(input)) {
    if (key === 'useProxy') { if (typeof value !== 'boolean') throw new CliError('useProxy must be boolean') }
      ...
      text(value, key)
      if (key === 'baseUrl' || (key === 'relayUrl' && value)) url(value, key)
      if (key === 'model' && !value.trim()) throw new CliError('model cannot be empty')
      if (key === 'extraHeaders') {
        try { new Headers(parseExtra(value)) } catch { throw new CliError('extraHeaders must be a JSON object of valid string headers') }
      }
    }
  }
}
export function validateProfile(input: unknown): asserts input is Profile {
  object(input, 'profile'); keys(input, ['settings', 'params', 'apiKeyEnv', 'outputDir'], 'profile')
  if (input.settings !== undefined) validateSettings(input.settings)
  if (input.params !== undefined) validateParams(input.params)
  if (input.apiKeyEnv !== undefined) { text(input.apiKeyEnv, 'apiKeyEnv'); if (input.apiKeyEnv && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(input.apiKeyEnv)) throw new CliError('Invalid environment variable name') }
  if (input.outputDir !== undefined) { text(input.outputDir, 'outputDir'); if (!input.outputDir.trim()) throw new CliError('outputDir cannot be empty') }
}
...
export function validateConfig(c: unknown): asserts c is Config {
  object(c, 'config'); keys(c, Object.keys(defaults()), 'config')
  if (c.version !== 1) throw new CliError('Unsupported config version')
  integer(c.concurrency, 1, 100, 'concurrency'); integer(c.generationTimeout, 1, 86400, 'generationTimeout'); integer(c.downloadTimeout, 1, 86400, 'downloadTimeout')
  text(c.outputDir, 'outputDir'); if (!c.outputDir.trim()) throw new CliError('outputDir cannot be empty')
  validateProfile(c.defaults); object(c.profiles, 'profiles')
  for (const [name, value] of Object.entries(c.profiles)) { profileName(name); validateProfile(value) }
...
export function resolveSpec(config: Config, input: unknown) {
  ...
  if (spec.mode === 'edit' && !images.length) throw new CliError('edit requires at least one image')
  if (spec.mode === 'generate' && (images.length || spec.mask)) throw new CliError('Reference images and masks require edit mode')
