...
import type { ImagesResponse, GenerateBody } from '../src/lib/api.js'
...
export async function runWorker(home: string) {
  ...
  try {
    ...
    const saveJob = (job: Job) => atomicJson(join(jobDir(home, job.id), 'job.json'), job)
    const jobView = (job: Job) => publicValue(job)
    const profileFor = (name: string) => mergeProfile(config.defaults, config.profiles[name] ?? {})
    const keyFor = (name: string, env?: string) => env ? environment[env] || '' : profileFor(name).settings?.apiKey || ''
    function scrub(message: string, job?: Job, actualKey?: string) {
    ...
    for (const job of jobs.values()) {
      if (job.status === 'running') {
        ...
        if (!exists) { job.error = { code: 'INTERRUPTED', message: 'Generation outcome unknown; explicit retry required', phase: 'generation' }; job.finishedAt = new Date().toISOString() }
        await saveJob(job)
      }
    }
    async function execute(job: Job, controller: AbortController) {
      ...
      try {
        ...
        if (job.status === 'downloading') response = await readJson(join(jobDir(home, job.id), 'response.json'))
          ...
          job.status = 'downloading'; await saveJob(job); phase = 'download'
        }
        if (!response.data?.length) throw new Error('No final images returned')
        ...
        for (const [index, image] of response.data.entries()) {
          ...
          try {
            ...
            if (saved && await checkInput(saved).then(() => true, () => false)) continue
            ...
            await saveJob(job)
          } catch (error) { controller.signal.throwIfAborted(); failures.push(`Image ${index + 1}: ${messageOf(error)}`) }
        }
        if (failures.length) throw new Error(failures.join('; '))
        if (response.data.length !== job.params.n) throw new Error(`Expected ${job.params.n} final images, received ${response.data.length}`)
        controller.signal.throwIfAborted()
        job.status = 'succeeded'; job.finishedAt = new Date().toISOString(); delete job.error
    ...
    function pump() {
      if (stopping) { if (!active.size) void close(); return }
      for (const job of jobs.values()) {
    ...
    function find(id: unknown) {
      const job = jobs.get(String(id)); if (!job) throw new CliError('Unknown job', 'NOT_FOUND'); return job
    }
    async function submit(input: unknown, parent?: Job, publish = true) {
      ...
      try {
        ...
        if (parent) {
          ...
          if (await stat(join(jobDir(home, parent.id), 'response.json')).then(() => true, () => false)) {
            ...
            await atomicJson(join(dir, 'response.json'), response); job.status = 'downloading'; job.usage = parent.usage
            for (const path of parent.files) {
              if (!await checkInput(path).then(() => true, () => false)) continue
              await mkdir(job.outputDir, { recursive: true })
              const target = join(job.outputDir, basename(path))
              await copyFile(path, target); job.files.push(target)
            }
          }
        }
        await saveJob(job)
        if (publish) jobs.set(id, job)
        return job
      } catch (error) { await rm(dir, { recursive: true, force: true }); throw error }
    }
    async function saveConfig(next: unknown) {
      validateConfig(next); await atomicJson(join(home, 'config.json'), next); config = next
    ...
    async function handle(command: string, args: Record<string, unknown>) {
      switch (command) {
        ...
        case 'stop': stopping = true; return { stopping: true }
        case 'config.get': return publicValue(config)
        case 'config.init': return saveConfig(config)
        case 'config.set': return saveConfig({ ...config, ...args, defaults: args.defaults ? mergeProfile(config.defaults, args.defaults as Profile) : config.defaults })
        case 'config.import': {
        ...
        case 'profile.set': {
          const name = String(args.name); profileName(name); validateProfile(args.profile)
        ...
        case 'profile.remove': {
          ...
          if (name === config.defaultProfile) throw new CliError('Cannot remove the default profile')
          if ([...jobs.values()].some(j => j.profile === name && !terminal(j.status))) throw new CliError('Profile has unfinished jobs')
          const profiles = { ...config.profiles }; delete profiles[name]; return saveConfig({ ...config, profiles })
        }
        case 'models': {
          ...
          if (!apiKey) throw new CliError('Missing API key')
          ...
          if (response.status === 404 && args.probe) return { reachable: true, modelsAvailable: false }
          if (!response.ok) throw new CliError(`Models endpoint HTTP ${response.status}`, 'UPSTREAM_ERROR', 1)
          const body = await response.json() as { data?: { id: string }[] }
          return { reachable: true, models: (body.data ?? []).map(m => m.id) }
        }
        case 'submit': return jobView(await submit(args.spec))
        case 'batch': {
          if (!Array.isArray(args.specs) || !args.specs.length) throw new CliError('Batch must be a nonempty JSON array')
          for (const spec of args.specs) {
            const r = resolveSpec(config, spec)
            for (const path of r.images) await checkInput(path)
            if (spec.mask) await checkInput(spec.mask, true)
          }
          const added: Job[] = []
          try {
            for (const spec of args.specs) added.push(await submit(spec, undefined, false))
            for (const job of added) jobs.set(job.id, job)
            return added.map(jobView)
          }
          catch (error) { for (const job of added) { jobs.delete(job.id); await rm(jobDir(home, job.id), { recursive: true, force: true }) }; throw error }
        }
        ...
        case 'jobs.cancel': {
          ...
          if (!terminal(job.status)) { job.status = 'cancelled'; job.finishedAt = new Date().toISOString(); active.get(job.id)?.abort(); await saveJob(job) }
          return jobView(job)
        }
        case 'jobs.retry': {
          const parent = find(args.id)
          if (!['failed', 'cancelled', 'interrupted'].includes(parent.status) || active.has(parent.id)) throw new CliError('Retry requires a finished failed, cancelled or interrupted job')
    ...
    const server = createServer(async (req, res) => {
      ...
      try {
        ...
        for await (const chunk of req) { size += chunk.length; if (size > 16 * 1024 * 1024) throw new CliError('IPC request too large'); chunks.push(chunk) }
        const input = JSON.parse(Buffer.concat(chunks).toString('utf8')) as { command: string; args?: Record<string, unknown>; env?: Record<string, string> }
