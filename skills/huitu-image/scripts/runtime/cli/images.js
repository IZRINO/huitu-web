...
export async function checkInput(path, mask = false) {
    const info = await stat(path).catch(() => { throw new CliError(`Cannot read image: ${path}`); });
    if (!info.isFile() || info.size > 50 * 1024 * 1024)
        throw new CliError(`Image must be a file no larger than 50 MiB: ${path}`);
    const bytes = await readFile(path);
    const format = imageFormat(bytes);
    if (mask && format !== 'png')
        throw new CliError('Mask must be PNG');
    return format;
}
export async function stageInputs(dir, images, mask) {
    ...
    let maskPath;
    if (mask) {
        await checkInput(mask, true);
        maskPath = join(inputs, 'mask.png');
        await copyFile(mask, maskPath);
        await checkInput(maskPath, true);
    }
    return { images: paths, mask: maskPath };
}
export async function imageFile(path) {
...
async function obtain(image, signal, timeout) {
    if (image.b64_json || image.result)
        return decode(image.b64_json || image.result);
    if (!image.url)
        throw new Error('Missing image data');
    if (image.url.startsWith('data:'))
        return decode(image.url);
...
export async function downloadImage(image, dir, index, signal, timeout) {
