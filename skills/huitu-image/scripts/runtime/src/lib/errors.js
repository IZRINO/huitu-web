...
export function describeHttp(status, bodyText) {
    let parsed = null;
    try {
    ...
    if (status === 401)
        return msg || '密钥被拒绝，检查中转站与密钥';
    if (status === 403)
        return msg || '没有权限调用该模型';
    if (status === 404)
        return msg || '接口路径不对，确认地址填到 /v1';
    if (status === 413)
        return msg || '图片太大';
    if (status === 429)
        return msg || '触发限流，稍后再曝';
    if (status >= 500)
        return msg || `中转站 ${status}`;
