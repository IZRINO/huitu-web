...
export function b64ToDataUrl(b64: string, format: string): string {
...
export function stampName(prefix: string, size: string, format: string): string {
  const t = new Date()
  const p = [
    t.getFullYear(),
...
export function estimateCost(usage?: Usage): string | null {
  ...
  if (usd <= 0 && !usage.total_tokens) return null
  return `$${usd.toFixed(4)}`
}

export function usageLine(usage?: Usage): string | null {
  if (!usage) return null
  const parts: string[] = []
