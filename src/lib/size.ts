...
export const LONG_EDGES = [1024, 1536, 2048, 2560, 3840] as const
...
export function sizeFromAspect(aspectId: string, longEdge: number): string {
...
export function validateSize(size: string): { ok: true; w?: number; h?: number } | { ok: false; message: string } {
  if (size === 'auto') return { ok: true }
  const parsed = parseSize(size)
  if (!parsed) return { ok: false, message: '分辨率写成 宽x高，例如 1536x1024' }
  const { w, h } = parsed
  if (w % 16 !== 0 || h % 16 !== 0) return { ok: false, message: '宽和高都必须是 16 的倍数' }
  if (w > MAX_EDGE || h > MAX_EDGE) return { ok: false, message: `长边不能超过 ${MAX_EDGE}` }
  if (w < 16 || h < 16) return { ok: false, message: '边长过短' }
  const ratio = Math.max(w, h) / Math.min(w, h)
  if (ratio > MAX_RATIO + 1e-6) return { ok: false, message: '长宽比不能超过 3:1' }
  const px = w * h
  if (px < MIN_PIXELS) return { ok: false, message: `总像素不能低于 ${MIN_PIXELS}` }
  if (px > MAX_PIXELS) return { ok: false, message: `总像素不能超过 ${MAX_PIXELS}` }
  return { ok: true, w, h }
}
...
export function resolveSize(p: {
  ...
  if (p.sizeMode === 'auto') return 'auto'
  if (p.sizeMode === 'preset') return p.sizePreset
  if (p.sizeMode === 'aspect') return sizeFromAspect(p.aspect, p.longEdge)
  return `${snap16(p.customW)}x${snap16(p.customH)}`
}

export function sheetRatio(size: string): number {
  const parsed = parseSize(size)
  if (!parsed) return 1
